    ...
    <AppShell>
      <div data-testid="estimates-list-page" className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button data-testid="create-estimate-btn" onClick={() => setShowCreate(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-white" style={{ background: 'linear-gradient(135deg, #0F2D5C, #0E7490)' }}><Plus size={16} weight="bold" /> New Estimate</button>
        </div>
                  ...
                  <tr key={e.estimate_id} style={{ background: i % 2 === 0 ? '#FFFFFF' : '#FAFBFC', borderBottom: '1px solid #F2F4F6' }}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        {e.status !== 'Converted' && <button onClick={() => handleConvert(e.estimate_id)} className="text-xs font-medium px-2 py-1 rounded hover:bg-[#F2F4F6]" style={{ color: '#0F2D5C' }} title="Convert to Invoice"><ArrowsClockwise size={14} /></button>}
          ...
          <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto py-8" style={{ background: 'rgba(25,28,30,0.5)' }}>
            <div className="rounded-2xl p-6 w-full max-w-3xl" style={{ background: '#FFFFFF' }}>
              ...
              <div className="space-y-2 mb-4">
                ...
                <button onClick={() => setForm({ ...form, items: [...form.items, { product: '', description: '', quantity: 1, unit: 'pcs', rate: 0, amount: 0 }] })} className="text-xs font-medium" style={{ color: '#0F2D5C' }}><Plus size={12} className="inline" /> Add Line</button>
              </div>
