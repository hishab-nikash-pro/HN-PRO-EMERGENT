...
import { ArrowLeft, Check, FileArrowUp, X } from '@phosphor-icons/react';
import ActivityTimeline from '../components/ActivityTimeline';
import AppShell from '../components/layout/AppShell';
import { useCompany } from '../contexts/CompanyContext';
import { approveRecord, convertSalesOrderToInvoice, getRecordActivity, getSalesOrder, rejectRecord, submitApproval, updateSalesOrder } from '../lib/api';
...
export default function SalesOrderDetail() {
  ...
  const load = async () => {
    if (!selectedCompany || !salesOrderId) return;
    setLoading(true);
    try {
      const [orderRes, activityRes] = await Promise.all([
        getSalesOrder(selectedCompany.company_id, salesOrderId),
        getRecordActivity(selectedCompany.company_id, 'sales_order', salesOrderId),
      ]);
  ...
  useEffect(() => { load(); }, [selectedCompany, salesOrderId]);
  ...
  if (!order) return <AppShell><div className="text-center py-12" style={{ color: '#64748B' }}>Sales order not found.</div></AppShell>;
  ...
  const canConfirm = can.write && order.status === 'Draft';
  const canConvert = can.write && !order.converted_invoice_id;
  const canSubmitApproval = can.write && approvalStatus === 'Draft';
  const canApproveOrReject = can.manage && approvalStatus === 'Submitted';

  return (
    <AppShell>
      <div className="space-y-6 max-w-6xl" data-testid="sales-order-detail-page">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/sales-orders')} className="p-2 rounded-lg hover:bg-white" style={{ color: '#475569' }}><ArrowLeft size={20} /></button>
            <div>
              <h1 className="text-2xl font-bold" style={{ fontFamily: 'Manrope, sans-serif', color: '#191C1E' }}>{order.sales_order_number}</h1>
              <p className="text-sm mt-1" style={{ color: '#475569' }}>{order.customer_name} • {order.status}</p>
            </div>
          </div>
          <div className="flex gap-2 flex-wrap">
            {canConfirm && <button disabled={actionLoading === 'confirm'} onClick={() => runAction('confirm', () => updateSalesOrder(selectedCompany.company_id, salesOrderId, { status: 'Confirmed' }), 'Sales order marked as confirmed.')} className="px-3 py-2 rounded-lg text-sm font-medium disabled:opacity-60" style={{ color: '#0F2D5C', boxShadow: '0 0 0 1px #CBD5E1' }}>{actionLoading === 'confirm' ? 'Saving...' : 'Mark Confirmed'}</button>}
            {order.converted_invoice_id ? (
              <button onClick={() => navigate(`/sales/${order.converted_invoice_id}`)} className="px-3 py-2 rounded-lg text-sm font-medium text-white" style={{ background: 'linear-gradient(135deg, #0F2D5C, #0E7490)' }}>Open Invoice</button>
            ) : canConvert ? (
              <button disabled={actionLoading === 'convert'} onClick={() => runAction('convert', async () => {
                const res = await convertSalesOrderToInvoice(selectedCompany.company_id, salesOrderId);
                navigate(`/sales/${res.data.invoice_id}`);
              }, 'Sales order converted to invoice.')} className="px-3 py-2 rounded-lg text-sm font-medium text-white disabled:opacity-60" style={{ background: 'linear-gradient(135deg, #0F2D5C, #0E7490)' }}>{actionLoading === 'convert' ? 'Converting...' : 'Convert to Invoice'}</button>
            ) : null}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="rounded-2xl p-6" style={{ background: '#FFFFFF', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              ...
              {order.shipping_address && <p className="text-sm mt-4" style={{ color: '#475569' }}>{order.shipping_address}</p>}
            </div>
                ...
                <tbody>
                  {(order.items || []).map((item, index) => (
                    <tr key={index} style={{ borderBottom: '1px solid #F1F5F9' }}>
                      ...
                      <td className="px-4 py-3 text-right" style={{ color: '#191C1E' }}>{item.quantity}</td>
                      <td className="px-4 py-3 text-right" style={{ color: '#191C1E' }}>${Number(item.rate || 0).toFixed(2)}</td>
                      <td className="px-4 py-3 text-right font-semibold" style={{ color: '#191C1E' }}>${Number(item.amount || 0).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
