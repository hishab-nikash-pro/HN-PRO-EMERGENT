...
import { listCustomerPayments } from '../lib/api';
import AppShell from '../components/layout/AppShell';
import { useNavigate } from 'react-router-dom';
import { Plus, MagnifyingGlass, CurrencyDollar, Receipt, CaretRight } from '@phosphor-icons/react';
...
export default function CustomerPayments() {
  ...
  const filtered = payments.filter(p => {
    ...
    const matchQ = !q || (p.customer_name || '').toLowerCase().includes(q) || (p.invoice_number || '').toLowerCase().includes(q) || (p.reference || '').toLowerCase().includes(q);
  ...
  return (
    <AppShell>
      <div data-testid="customer-payments-page" className="space-y-5 md:space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold" style={{ fontFamily: 'Manrope, sans-serif', color: '#0F172A' }}>Customer Payments</h1>
            <p className="text-sm mt-1" style={{ color: '#475569' }}>Payments received from customers against invoices</p>
          </div>
          <button
            data-testid="new-customer-payment-btn"
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
          <div className="rounded-xl p-4 md:p-5" style={{ background: '#FFFFFF', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            <p className="text-[11px] font-medium uppercase tracking-wider" style={{ color: '#475569' }}>Total Received</p>
            <p className="text-lg md:text-2xl font-bold mt-1 tabular-nums" style={{ fontFamily: 'Manrope, sans-serif', color: '#0F2D5C' }}>{money(totalReceived)}</p>
          </div>
          <div className="rounded-xl p-4 md:p-5" style={{ background: '#FFFFFF', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        ...
        <div className="space-y-3">
          <DateFilterPreset 
            ...
            storageKey="customer_payments_date_filter"
            defaultPreset="this_month"
          />
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
            <div className="relative flex-1 sm:max-w-xs">
              <MagnifyingGlass size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#475569' }} />
              <input data-testid="payments-search" type="text" placeholder="Search by customer, invoice, ref..." value={search}
            ...
            <select data-testid="payment-method-filter" value={method}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: '#F7F9FB', borderBottom: '1px solid #CBD5E1' }}>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider" style={{ color: '#475569' }}>Customer</th>
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    <tr key={p.payment_id || i} data-testid={`payment-row-${p.payment_id || i}`}
                      onClick={() => p.invoice_id && navigate(`/sales/${p.invoice_id}`)}
                      className="transition-colors hover:bg-[#F7F9FB] cursor-pointer"
                      ...
                      <td className="px-4 py-3 font-medium" style={{ color: '#0F172A' }}>{p.customer_name || '—'}</td>
                      <td className="px-4 py-3" style={{ color: '#0E7490' }}>{p.invoice_number || '—'}</td>
                      <td className="px-4 py-3" style={{ color: '#475569' }}>{p.payment_method || '—'}</td>
                      <td className="px-4 py-3" style={{ color: '#475569' }}>{p.reference || '—'}</td>
                      <td className="px-4 py-3 text-right font-semibold tabular-nums" style={{ fontFamily: 'Manrope, sans-serif', color: '#047857' }}>
                        {money(p.amount)}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="md:hidden space-y-2">
          {loading ? (
            ...
            <div key={p.payment_id || i} onClick={() => p.invoice_id && navigate(`/sales/${p.invoice_id}`)}
              className="rounded-xl p-4 flex items-center justify-between active:bg-[#F7F9FB] transition-colors"
              style={{ background: '#FFFFFF', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div className="min-w-0 flex-1">
                <p className="font-semibold truncate" style={{ color: '#0F172A' }}>{p.customer_name || '—'}</p>
                <p className="text-xs mt-0.5" style={{ color: '#475569' }}>{p.payment_date} • {p.invoice_number || '—'}</p>
                <p className="text-[11px] mt-0.5" style={{ color: '#475569' }}>{p.payment_method}{p.reference ? ` • ${p.reference}` : ''}</p>
              </div>
              <div className="text-right flex items-center gap-2">
                <span className="font-bold tabular-nums" style={{ fontFamily: 'Manrope, sans-serif', color: '#047857' }}>{money(p.amount)}</span>
                <CaretRight size={14} style={{ color: '#CBD5E1' }} />
              </div>
            </div>
          ))}
        </div>
